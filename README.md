...
**동일한 기능도 플랫폼별 파일 처리 방식이 달랐음**
    
같은 인증 이미지 업로드 플로우라도 iOS에서는 정상 동작했지만 Android에서는 마지막 제출 단계에서만 실패하는 문제가 있었습니다. 공통 UI와 공통 비즈니스 로직을 유지하더라도, 이미지 선택·파일 URI·업로드처럼 네이티브 경계에 가까운 기능은 플랫폼별 차이를 별도로 확인해야 했습니다.
