...
### iOS 개발자가 RN을 사용하며 느낀 점

iOS 네이티브 개발을 먼저 경험한 상태에서 RN 프로젝트를 진행하면서, 공통 코드의 장점은 분명했지만 플랫폼과 맞닿는 경계에서는 여전히 각 환경을 따로 이해해야 한다는 점을 확인했습니다. 화면 구조와 비즈니스 로직은 상당 부분 공통으로 가져갈 수 있었지만, 키보드, 권한, 로그인, 딥링크처럼 네이티브 동작과 연결되는 영역에서는 플랫폼별 대응이 필요했습니다.

**화면 생명주기 — `viewDidAppear`에 가장 가까운 흐름**

그룹 참여 화면에 진입하면 초대 코드 입력 필드가 자동으로 포커스되고 키보드가 올라오도록 구현했습니다. iOS에서는 보통 `viewDidAppear` 시점에 `becomeFirstResponder()`를 호출하는 방식으로 처리할 수 있었지만, RN에서는 `useFocusEffect`가 화면이 실제로 다시 보이는 시점을 다루는 데 가장 가까운 도구였습니다.

다만 같은 `focus()` 호출이라도 플랫폼별 차이가 있었습니다. iOS에서는 바로 호출해도 동작했지만, Android에서는 화면 전환 애니메이션이 끝나기 전에 실행되면 포커스가 무시되는 경우가 있었습니다. 이 문제는 `InteractionManager.runAfterInteractions()`으로 감싸서 애니메이션이 끝난 뒤 실행하도록 처리했습니다.
...
| 기능 | iOS | Android |
|------|-----|---------|
| 화면 진입 시 키보드 포커스 | `useFocusEffect` + 즉시 `focus()`로 동작 | 애니메이션 이후 `InteractionManager`로 지연 호출 필요 |
| 키보드 레이아웃 처리 | `KeyboardAvoidingView`의 `padding` 계열이 비교적 자연스러움 | `height` 기반 처리가 더 안정적인 경우가 많음 |
| 카카오 로그인 복귀 | URL scheme 설정 중심 | `AndroidManifest.xml`의 `intent-filter` 설정 필요 |
| 이미지 피커 권한 | 비교적 단순한 권한 흐름 | OS 버전에 따라 권한 처리 흐름이 달라짐 |
| 그림자 스타일 | `shadow*` 속성 사용 | `elevation` 중심 처리 |

**앱 생명주기 — 포그라운드 / 백그라운드 상태 관리**

iOS에서는 `applicationDidBecomeActive`, `applicationDidEnterBackground` 같은 생명주기 메서드로 앱 상태를 다룹니다. RN에서는 `AppState`가 이에 대응하며, `'active'`, `'background'`, `'inactive'` 상태를 구독할 수 있습니다. 구조는 비슷하지만, 실제로는 Android에서 `active/background` 중심으로 상태를 확인하는 경우가 많았습니다.
...
**공통 로직 재사용**  
Repository, UseCase, 상태 관리, 에러 처리 같은 핵심 로직은 하나의 코드베이스 안에서 공통으로 유지할 수 있었습니다. 플랫폼 차이는 주로 권한, 키보드, 로그인 복귀, 이미지 선택처럼 네이티브 경계에서 발생했고, 그 외의 구조와 흐름은 공통으로 가져갈 수 있었습니다.

**개발 속도**  
Fast Refresh 덕분에 단순 UI 수정은 재빌드 없이 빠르게 확인할 수 있었습니다. 화면 구조를 조정하거나 스타일을 다듬는 작업에서는 피드백 사이클이 짧아지는 장점이 분명했습니다.

**Android 확장 비용 감소**  
iOS 기준으로 설계한 화면 구조와 상태 관리 흐름을 Android까지 확장하는 비용이 네이티브 앱을 각각 따로 구현하는 경우보다 낮았습니다. 특히 공통 로직과 화면 플로우를 재사용할 수 있다는 점에서 크로스플랫폼의 장점이 분명했습니다.
